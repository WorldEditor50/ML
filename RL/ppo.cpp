...
void ML::PPO::learnWithKLpenalty(OptType optType, double learningRate, std::vector<ML::Transition> &x)
{
    /* reward */
    int end = x.size() - 1;
...
void ML::PPO::learnWithClipObject(OptType optType, double learningRate, std::vector<ML::Transition> &x)
{
    /* reward */
    int end = x.size() - 1;
    double r = getValue(x[end].state);
