#include "bayes.h"
void ML::Bayes::splitString(std::string& strSrc,
...
void ML::Bayes::convertDataToFeature(const std::vector<std::string>& xsi, std::vector<int>& xi)
{
    if (elements.empty()) {
        return;
    }
...
void ML::Bayes::load(const std::string& fileName)
{
    /* load data from file */
    std::string strRow;
    std::ifstream file;
...
int ML::Bayes::indicator(int x_id, int y_id)
{
    int num = 0;
...
void ML::Bayes::calculatePosterior(std::vector<int>& xi)
{
...
std::string ML::Bayes::classify(std::string& sample)
{
    int index = 0;
    std::vector<std::string> strVect;
